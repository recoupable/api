...
- A separate `collectSpotifyReleaseTrackIsrcs` source reader can look up the observed track IDs from a verified release page, at most 100 slots per call and five concurrent Spotify requests. Repeated positions share one lookup. It records source URL, response status and missing/failed ISRC gaps without retries or identity writes. This reader is fixture-tested but **not connected to the durable workflow**; no release position becomes a recording subject automatically.
