...
describe("creditsForCompletedGeneration", () => {
  it("charges the actual output length, not the requested one", () => {
    ...
    // itself pass-through.
    expect(creditsForCompletedGeneration({ requestedSeconds: 60, actualSeconds: 25.87 })).toBe(6);
  });

  it("charges the request when the model runs the full length", () => {
    expect(creditsForCompletedGeneration({ requestedSeconds: 60, actualSeconds: 60 })).toBe(12);
  });
  ...
  it("never charges above what the caller was quoted", () => {
    ...
    expect(creditsForCompletedGeneration({ requestedSeconds: 60, actualSeconds: 60.07 })).toBe(12);
  });
  ...
  it("falls back to the requested duration when fal reports no length", () => {
    ...
    expect(creditsForCompletedGeneration({ requestedSeconds: 60, actualSeconds: null })).toBe(12);
  });

  it("treats a nonsensical zero-length result as unknown rather than free", () => {
    expect(creditsForCompletedGeneration({ requestedSeconds: 60, actualSeconds: 0 })).toBe(12);
  });
});
