...
describe("creditsForCompletedGeneration", () => {
  it("charges the actual output length, not the requested one", () => {
    ...
    // itself pass-through.
    expect(creditsForCompletedGeneration({ requestedSeconds: 60, actualSeconds: 25.87 })).toBe(51_740);
  });

  it("charges the request when the model runs the full length", () => {
    expect(creditsForCompletedGeneration({ requestedSeconds: 60, actualSeconds: 60 })).toBe(120_000);
  });
  ...
  it("never charges above what the caller was quoted", () => {
    ...
    expect(creditsForCompletedGeneration({ requestedSeconds: 60, actualSeconds: 60.07 })).toBe(120_000);
  });
  ...
  it("falls back to the requested duration when fal reports no length", () => {
    ...
    expect(creditsForCompletedGeneration({ requestedSeconds: 60, actualSeconds: null })).toBe(120_000);
  });
