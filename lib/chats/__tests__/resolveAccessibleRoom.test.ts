...
describe("resolveAccessibleRoom", () => {
  ...
  it("returns 400 when roomId is invalid uuid", async () => {
    const result = await resolveAccessibleRoom(request, "invalid-id");
  ...
  it("returns auth response when auth fails", async () => {
    ...
    const result = await resolveAccessibleRoom(request, roomId);
    expect(result).toBeInstanceOf(NextResponse);
    const response = result as NextResponse;
  ...
  it("returns room and accountId for accessible room", async () => {
    const room = {
      id: roomId,
