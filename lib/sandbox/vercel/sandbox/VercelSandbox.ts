...
${hostLine}${portLines}${runtimeEnvLine}`;
  ...
  private getRoutePorts(): number[] {
    ...
    return routes.map(route => route.port).filter(port => Number.isInteger(port) && port > 0);
  }
  ...
  private getCommandEnv(perCallEnv?: Record<string, string>): Record<string, string> | undefined {
    const runtimePreviewEnv = this.getRuntimePreviewEnv();
    const hasPerCall = perCallEnv && Object.keys(perCallEnv).length > 0;
    if (!this.env && Object.keys(runtimePreviewEnv).length === 0 && !hasPerCall) {
      return undefined;
    }
  ...
  static async create(config: VercelSandboxConfig = {}): Promise<VercelSandbox> {
    ...
    if (source?.token) {
      const authenticatedUrl = buildAuthenticatedGitHubUrl(source.url, source.token);
      if (authenticatedUrl) {
        await sdk.runCommand({
          cmd: "git",
    ...
    if (!source && !restoreSnapshotId && gitUser && !skipGitWorkspaceBootstrap) {
      await sdk.runCommand({
        cmd: "git",
