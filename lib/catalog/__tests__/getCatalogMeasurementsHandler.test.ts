...
const okQuery = (query: Record<string, unknown> = { accountId, catalogId, page: 1, limit: 50 }) =>
  vi.mocked(validateGetCatalogMeasurementsQuery).mockResolvedValue({
    accountId,
    ...query,
  } as never);
...
describe("getCatalogMeasurementsHandler", () => {
  ...
  it("short-circuits with the validator error (auth + params live in the validator)", async () => {
    const err = NextResponse.json({ status: "error" }, { status: 401 });
    vi.mocked(validateGetCatalogMeasurementsQuery).mockResolvedValue(err);
