...
describe("startYoutubeProfileScraping", () => {
  ...
  it("requests plain-text auto-generated captions only when subtitles is true", async () => {
    await startYoutubeProfileScraping("mycowtf", 10, { origin: "artist" }, { subtitles: true });
    expect(start).toHaveBeenCalledWith(
      expect.objectContaining({
        maxResults: 10,
  ...
  it("leaves captions off by default (dataset items stay small for every other caller)", async () => {
    await startYoutubeProfileScraping("mycowtf", 10);
    expect(start).toHaveBeenCalledWith(
      expect.objectContaining({ downloadSubtitles: false, subtitlesFormat: "srt" }),
      { webhooks: expect.any(Array) },
    );
  });
