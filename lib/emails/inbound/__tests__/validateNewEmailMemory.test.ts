...
describe("validateNewEmailMemory", () => {
  ...
  it("returns senderEmail resolved from the raw From header so callers can reply to the right address", async () => {
    vi.mocked(getEmailContent).mockResolvedValue({
    ...
    expect(result).not.toHaveProperty("response");
    const { senderEmail } = result as { senderEmail: string };
    expect(senderEmail).toBe("sweetmantech@gmail.com");
  });
