...
/**
 * Registers the "research_tracks" tool on the MCP server.
 * Returns all tracks by an artist with popularity data.
 *
 * @param server - The MCP server instance to register the tool on.
 */
export function registerResearchTracksTool(server: McpServer): void {
  server.registerTool(
    "get_artist_tracks",
    {
      description:
        "Get all tracks by an artist with popularity data. Accepts artist name. For Spotify top 10 tracks with preview URLs, use get_spotify_artist_top_tracks instead.",
      inputSchema: schema,
    },
    async (args, extra: RequestHandlerExtra<ServerRequest, ServerNotification>) => {
      ...
      try {
        ...
        const result = await proxyToChartmetric(`/artist/${resolved.id}/tracks`);
