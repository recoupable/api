...
/**
 * Registers the "research_urls" tool on the MCP server.
 * Returns all social and streaming URLs for an artist.
 *
 * @param server - The MCP server instance to register the tool on.
 */
export function registerResearchUrlsTool(server: McpServer): void {
  server.registerTool(
    "get_artist_urls",
    {
      description:
        "Get all known social and streaming URLs for any artist by name — Spotify, Instagram, TikTok, YouTube, Twitter, SoundCloud, and more. For socials connected to a Recoup artist account, use get_artist_socials instead.",
      inputSchema: schema,
    },
    async (args, extra: RequestHandlerExtra<ServerRequest, ServerNotification>) => {
      ...
      try {
        ...
        return getToolResultError(error instanceof Error ? error.message : "Failed to fetch URLs");
      }
    },
  );
}
