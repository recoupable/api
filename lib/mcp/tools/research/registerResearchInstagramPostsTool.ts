...
/**
 * Registers the "research_instagram_posts" tool on the MCP server.
 * Returns an artist's top Instagram posts and reels sorted by engagement.
 *
 * @param server - The MCP server instance to register the tool on.
 */
export function registerResearchInstagramPostsTool(server: McpServer): void {
  server.registerTool(
    "get_artist_instagram_posts",
    {
    ...
    async (args, extra: RequestHandlerExtra<ServerRequest, ServerNotification>) => {
      ...
      try {
        ...
        return getToolResultError(
          error instanceof Error ? error.message : "Failed to fetch Instagram posts",
        );
      }
    },
  );
