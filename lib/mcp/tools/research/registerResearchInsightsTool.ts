...
/**
 * Registers the "research_insights" tool on the MCP server.
 * Returns AI-generated insights about an artist — trends, milestones, and observations.
 *
 * @param server - The MCP server instance to register the tool on.
 */
export function registerResearchInsightsTool(server: McpServer): void {
  server.registerTool(
    "get_artist_insights",
    {
      description:
    ...
    async (args, extra: RequestHandlerExtra<ServerRequest, ServerNotification>) => {
      ...
      try {
        ...
        const result = await proxyToChartmetric(`/artist/${resolved.id}/noteworthy-insights`);
