...
describe("registerOnNewMention", () => {
  ...
  it("includes lipsync and batch info in acknowledgment message", async () => {
    ...
    const cardBody = vi.mocked(thread.post).mock.calls[0][0] as string;
  ...
  it("posts only one plain-text message on success (no View Task link)", async () => {
    const bot = createMockBot();
    registerOnNewMention(bot as never);
    ...
    const posted = vi.mocked(thread.post).mock.calls[0][0] as string;
    expect(posted).toContain("Content Generation Started");
    expect(posted).not.toContain("/tasks/");
    expect(posted).not.toContain("run-abc-123");
  });
  ...
  it("posts a plain-text acknowledgment after triggering", async () => {
    ...
    expect(thread.post).toHaveBeenCalledWith(expect.stringContaining("test-artist"));
