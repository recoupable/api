...
export async function resolveSpotifyRelease(link: string) {
  ...
  if (!match) throw new Error("Paste a Spotify track, album, or playlist link.");
  ...
  if (!response.ok)
    throw new Error("Spotify couldn’t find that release. Check the link and try again.");
