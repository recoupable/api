...
it("passes actual assets and generates behavior, not just art direction", async () => {
  ...
  expect(JSON.stringify(ai.generateObject.mock.calls[0][0].messages)).toContain(site.assets[0].url);
  expect(ai.generateObject.mock.calls[0][0].system).toContain("playable mechanics");
});
