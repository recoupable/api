...
it("normalizes a shared Spotify link and retrieves real release artwork", async () => {
  ...
  expect(await resolveSpotifyRelease("https://open.spotify.com/intl-en/track/abc?si=123")).toEqual({
    title: "A release",
...
it("ignores artwork outside Spotify's image hosts", async () => {
  ...
  expect((await resolveSpotifyRelease("https://open.spotify.com/album/abc")).artwork).toBeNull();
});
it("reports unavailable releases without fabricating metadata", async () => {
  vi.stubGlobal("fetch", vi.fn().mockResolvedValue(new Response(null, { status: 404 })));
  await expect(resolveSpotifyRelease("https://open.spotify.com/track/abc")).rejects.toThrow(
    "couldn’t find",
  );
});
