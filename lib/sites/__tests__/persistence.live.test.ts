...
it.skipIf(process.env.SITES_DB_TEST !== "1")(
  ...
  async () => {
    ...
    const { selectSignups } = await import("@/lib/supabase/sites/selectSignups");
    const { GET } = await import("@/app/api/sites/public/[id]/route");
    const { POST } = await import("@/app/api/sites/public/[id]/signup/route");
    ...
    try {
      ...
      expect(await updateSite(site.id, owner, site.revision, { draft: snapshot })).toBeNull();
      ...
      expect((await anon.from("sites").select("id").eq("id", site.id)).error).toBeTruthy();
      expect(
        (await anon.from("site_signups").select("email").eq("site_id", site.id)).error,
      ).toBeTruthy();
