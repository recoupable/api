...
export const createAgentTemplateBodySchema = z.object({
  title: z
...
export type CreateAgentTemplateBody = z.infer<typeof createAgentTemplateBodySchema>;

/**
 * Validates the JSON body for POST /api/agent-templates.
 *
 * @param body - The raw JSON body
 * @returns A NextResponse with a 400 error or the parsed body on success.
 */
export function validateCreateAgentTemplateBody(
  body: unknown,
): NextResponse | CreateAgentTemplateBody {
  const result = createAgentTemplateBodySchema.safeParse(body);
