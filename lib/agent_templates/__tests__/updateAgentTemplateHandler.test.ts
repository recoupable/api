...
vi.mock("@/lib/agent_templates/validateUpdateAgentTemplateRequest", () => ({
  validateUpdateAgentTemplateRequest: vi.fn(),
}));
...
const { validateUpdateAgentTemplateRequest } = await import(
  "@/lib/agent_templates/validateUpdateAgentTemplateRequest"
);
...
describe("updateAgentTemplateHandler", () => {
  ...
  it("returns the validator error response when validation fails", async () => {
    ...
    vi.mocked(validateUpdateAgentTemplateRequest).mockResolvedValue(failure);
