...
describe("updateAutoTopUpHandler", () => {
  it("saves enabled settings when a card is on file and returns them in cents", async () => {
    ...
    vi.mocked(updateAutoTopUp).mockResolvedValue(storedRow);
    ...
    expect(updateAutoTopUp).toHaveBeenCalledWith({
      accountId: ACCOUNT,
      enabled: true,
  ...
  it("skips the card check when disabling and still stores the amounts", async () => {
    ...
    vi.mocked(updateAutoTopUp).mockResolvedValue({
      ...storedRow,
  ...
  it("returns 500 when the write throws", async () => {
    ...
    vi.mocked(updateAutoTopUp).mockRejectedValue(new Error("db down"));
