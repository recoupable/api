import { FLAMINGO_MINIMUM_CREDITS } from "@/lib/flamingo/creditsForFlamingoCall";
...
/**
 * The least an analyze request can possibly cost, for the pre-flight credit
 * gate: one floor per model call the request will make. `full_report` fans
 * out to one call per section; everything else is a single call. The real
 * charge, priced on the model's reported seconds, lands after the call.
 *
...
export function minimumCreditsForAnalyzeRequest(params: Partial<FlamingoGenerateBody>): number {
  ...
  return calls * FLAMINGO_MINIMUM_CREDITS;
}
