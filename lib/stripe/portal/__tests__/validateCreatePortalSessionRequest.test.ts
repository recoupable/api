...
describe("validateCreatePortalSessionRequest", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("returns 400 { error } for invalid JSON", async () => {
    ...
    const res = await validateCreatePortalSessionRequest(req);
  ...
  it("returns accountId and returnUrl when auth succeeds", async () => {
    ...
    const out = await validateCreatePortalSessionRequest(req);
    expect(out).toEqual({
      accountId: ACCOUNT,
      returnUrl: "https://chat.recoupable.com/back",
    });
