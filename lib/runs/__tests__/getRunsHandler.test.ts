...
describe("getRunsHandler", () => {
  ...
  it("returns the caller's latest run mapped to the run resource", async () => {
    ...
    expect(selectPlaycountSnapshots).toHaveBeenCalledWith({ account: accountId, limit: 1 });
  ...
  it("passes the requested limit through", async () => {
    ...
    expect(selectPlaycountSnapshots).toHaveBeenCalledWith({ account: accountId, limit: 5 });
  });
  ...
  it("returns 500 when the snapshot read fails, never an empty run list", async () => {
    ...
    vi.mocked(selectPlaycountSnapshots).mockRejectedValue(new Error("db down"));
