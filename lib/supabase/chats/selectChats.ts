 ...
 * `selectSessions` pattern: returns `null` on Supabase error (so callers
 * can distinguish a backend failure from a legitimately empty result),
 * and `[]` when the query succeeds but matches no rows.
 *
