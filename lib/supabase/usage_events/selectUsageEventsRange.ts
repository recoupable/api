...
interface SelectUsageEventsRangeParams {
  accountId?: string;
 ...
 */
export async function selectUsageEventsRange(
  params: SelectUsageEventsRangeParams,
  ...
  if (error) {
    console.error("Error selecting usage_events range:", error);
    throw error;
  }
  return data ?? [];
}
