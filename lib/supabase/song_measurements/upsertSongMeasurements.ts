...
/**
 * Upsert measurement rows, silently skipping rows whose
 ...
 */
export async function upsertSongMeasurements(
  ...
  if (error) {
    throw new Error(`Failed to upsert song measurements: ${error.message}`);
  }
