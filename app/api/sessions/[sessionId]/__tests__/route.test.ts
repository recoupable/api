...
const { selectSession } = await import("@/lib/supabase/sessions/selectSession");
...
describe("GET /api/sessions/[sessionId]", () => {
  ...
  it("returns 404 when session does not exist", async () => {
    ...
    vi.mocked(selectSession).mockResolvedValue(null);
  ...
  it("returns 403 when session is owned by a different account", async () => {
    ...
    vi.mocked(selectSession).mockResolvedValue(mockRow);
